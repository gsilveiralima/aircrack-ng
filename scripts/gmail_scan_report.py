...
def generate_report(output_path="relatorio_varredura_gmail_malware.pdf"):
    ...
    c.drawString(
        2 * cm,
        height - 2.6 * cm,
        f"Data: {datetime.now().strftime('%d/%m/%Y %H:%M')}  (Horário de Brasília)",
    )
    ...
    for f in findings:
        y = draw_wrapped_text(c, f"• Assunto: {f['subject']}", 2 * cm, y, 16 * cm)
        y = draw_wrapped_text(c, f"  Remetente: {f['sender']}", 2 * cm, y, 16 * cm)
        y = draw_wrapped_text(c, f"  Data: {f['date']}", 2 * cm, y, 16 * cm)
        y = draw_wrapped_text(c, f"  Observação: {f['risk']}", 2 * cm, y, 16 * cm)
        y -= 0.2 * cm
        if y < 3 * cm:
            c.showPage()
            y = height - 2 * cm
